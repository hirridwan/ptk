...
const FIELD = {
  ...
  "Pre-Test": "pre",
  "M1 Objektif": "m1",
  "M1 Uraian XP": "rubric_m1",
  "M2 Objektif": "m2",
  "M2 Uraian XP": "rubric_m2",
  "M3 Objektif": "m3",
  "M3 Uraian XP": "rubric_m3",
  "Boss Objektif": "boss",
  "Boss Uraian XP": "rubric_boss",
  "Post-Test": "post",
  "Total XP": "xp",
  "Jawaban M1": "answer_m1",
  "Jawaban M2": "answer_m2",
  "Jawaban M3": "answer_m3",
  "Jawaban Boss": "answer_boss",
  "Refleksi 3 Hal": "reflection_r3",
  "Refleksi 2 Hal": "reflection_r2",
  "Refleksi 1 Hal": "reflection_r1"
};
...
function getSheet_() {
  ...
  if (!sheet) {
    sheet = spreadsheet.insertSheet(DATA_SHEET);
  }
...
function setup() {
  const spreadsheet = SpreadsheetApp.openById(SHEET_ID);
  getSheet_();
  setupKelas_(spreadsheet);
  setupKelompok_(spreadsheet);
  refresh_();
...
function RESET_DATA_SISWA() {
  const spreadsheet = SpreadsheetApp.openById(SHEET_ID);
  const oldSheet = spreadsheet.getSheetByName(DATA_SHEET);
  ...
  if (oldSheet) {
    ...
    oldSheet.copyTo(spreadsheet).setName("Backup Data " + stamp);
    spreadsheet.deleteSheet(oldSheet);
  }

  const freshSheet = spreadsheet.insertSheet(DATA_SHEET);
  freshSheet.getRange(1, 1, 1, HEADERS.length)
    .setValues([HEADERS])
    .setFontWeight("bold");
  freshSheet.setFrozenRows(1);
  ...
  const records = rows
    ...
    .map(row => {
      ...
      HEADERS.forEach((header, index) => {
        record[FIELD[header] || header] = row[index];
      });

      record.pre = blankOrNumber_(record.pre);
      record.post = blankOrNumber_(record.post);

      ["m1", "m2", "m3", "boss", "xp"].forEach(key => {
        record[key] = numberOrZero_(record[key]);
      });

      ["rubric_m1", "rubric_m2", "rubric_m3", "rubric_boss"].forEach(key => {
        record[key] = record[key] === "" || record[key] === null || record[key] === undefined
          ? ""
          : numberOrZero_(record[key]);
      });
...
function doPost(e) {
  ...
  try {
    ...
    if (payload.action === "upsert") {
      return upsert_(payload.record || {});
    }

    if (payload.action === "grade") {
      return grade_(payload);
    }
...
function upsert_(record) {
  ...
  if (!className || !code || !name) {
    throw new Error("Kelas, kode siswa, dan nama wajib.");
  }

  if (!["X-5", "X-8", "X-11"].includes(className)) {
    throw new Error("Kelas tidak valid.");
  }

  const sheet = getSheet_();
  const existingRow = findRow_(sheet, className, code);
  const rowNumber = existingRow > 0
    ? existingRow
  ...
  if (existingRow > 0) {
    ...
    HEADERS.forEach((header, index) => {
      old[header] = oldValues[index];
    });
  }

  const textFields = [
    "Jawaban M1", "Jawaban M2", "Jawaban M3", "Jawaban Boss",
    "Refleksi 3 Hal", "Refleksi 2 Hal", "Refleksi 1 Hal"
  ];

  const rubricFields = [
    "M1 Uraian XP", "M2 Uraian XP", "M3 Uraian XP", "Boss Uraian XP"
  ];

  const row = HEADERS.map(header => {
    if (["N-Gain", "Kategori N-Gain", "Ketuntasan", "Total XP"].includes(header)) {
      return "";
    }
    ...
    if (textFields.includes(header) && String(incoming).trim() === "") {
      return old[header] ?? "";
    }

    if (rubricFields.includes(header) && String(incoming).trim() === "") {
      return old[header] ?? "";
    }
...
function grade_(payload) {
  ...
  if (!className || !code) {
    throw new Error("Kelas dan kode siswa wajib.");
  }
...
function findRow_(sheet, className, code) {
  ...
  for (let index = 0; index < values.length; index++) {
    const currentKey = String(values[index][0]).trim() + "|" + String(values[index][2]).trim();
    if (currentKey === key) return index + 2;
  }
...
function writeDerived_(sheet, rowNumber) {
  ...
  const totalXp = scoreColumns.reduce((sum, column) => {
    return sum + (Number(sheet.getRange(rowNumber, column).getValue()) || 0);
  }, 0);
...
function refresh_() {
  ...
  for (let row = 2; row <= lastRow; row++) {
    if (String(dataSheet.getRange(row, 3).getValue()).trim() !== "") {
      writeDerived_(dataSheet, row);
    }
  }

  updateKelas_(spreadsheet, dataSheet);
  updateKelompok_(spreadsheet, dataSheet);
}

function setupKelas_(spreadsheet) {
  let sheet = spreadsheet.getSheetByName(REKAP_KELAS);
  if (!sheet) sheet = spreadsheet.insertSheet(REKAP_KELAS);
...
function updateKelas_(spreadsheet, dataSheet) {
  setupKelas_(spreadsheet);
  const sheet = spreadsheet.getSheetByName(REKAP_KELAS);
  const lastRow = dataSheet.getLastRow();
  if (lastRow < 2) return;
  ...
  ["X-5", "X-8", "X-11"].forEach(className => {
    ...
    const tuntas = post.filter(value => value >= 80).length;
    ...
    output.push([
      ...
      tuntas,
      rows.length ? tuntas / rows.length : "",
      avg_(xp)
    ]);
  });
...
function setupKelompok_(spreadsheet) {
  let sheet = spreadsheet.getSheetByName(REKAP_KELOMPOK);
  if (!sheet) sheet = spreadsheet.insertSheet(REKAP_KELOMPOK);
...
function updateKelompok_(spreadsheet, dataSheet) {
  setupKelompok_(spreadsheet);
  const sheet = spreadsheet.getSheetByName(REKAP_KELOMPOK);
  const lastRow = dataSheet.getLastRow();
  if (lastRow < 2) return;
  ...
  ["X-5", "X-8", "X-11"].forEach(className => {
    for (let teamNumber = 1; teamNumber <= 6; teamNumber++) {
      ...
      output.push([
        ...
        avg_(post),
        avg_(nGain),
        post.filter(value => value >= 80).length,
        avg_(xp)
      ]);
    }
  });
